...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const appState = {
        ...
        storiesOffset: 0,
        isFetching: false,
        activeCategory: 'topstories', // 'clippings' is also a valid state now
        activeSort: 'default',
        ...
        maxScore: 0,
        maxComments: 0,
        savedStories: new Map(), // Map<number, Story>
        comments: {
            storyId: null,
            story: null,
            rootCommentIds: [],
    ...
    const LOCAL_STORAGE_KEY_SAVED = 'hn-times-clippings';
    ...
    const elements = {
        ...
        clippingsBtn: document.getElementById('clippings-btn'),
    ...
    function loadSavedStories() {
        try {
            ...
            if (saved) {
                ...
                appState.savedStories = new Map(parsed.map(s => [s.id, s]));
            }
        } catch (e) {
    ...
    function toggleSaveStory(story) {
        ...
        const btn = document.querySelector(`.clip-btn[data-story-id="${story.id}"]`);
        
        if (appState.savedStories.has(story.id)) {
            appState.savedStories.delete(story.id);
            if (btn) {
                btn.classList.remove('is-clipped');
                btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>`;
            }
            // If in clippings view, remove from grid immediately
            if (appState.activeCategory === 'clippings') {
                const cell = document.querySelector(`.story-cell .clip-btn[data-story-id="${story.id}"]`)?.closest('.story-cell');
                if (cell) cell.remove();
                if (appState.savedStories.size === 0) {
                     elements.grid.innerHTML = '<p class="no-results">No clipped stories yet.</p>';
                }
            }
        } else {
            appState.savedStories.set(story.id, story);
            if (btn) {
                btn.classList.add('is-clipped');
                btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>`;
            }
        }

        try {
            const arrayToSave = Array.from(appState.savedStories.values());
            localStorage.setItem(LOCAL_STORAGE_KEY_SAVED, JSON.stringify(arrayToSave));
        } catch (e) {
            console.error('Error saving to localStorage', e);
            alert('Could not save clipping (LocalStorage full?)');
        }
    }
    ...
    async function fetchStories(loadMore = false) {
        ...
        // Handle Clippings View specifically
        if (appState.activeCategory === 'clippings') {
            displayClippings();
            return;
        }
    ...
    function displayClippings() {
        ...
        const clippings = Array.from(appState.savedStories.values()).sort((a, b) => b.time - a.time);
        ...
        // For clippings, we treat them as "stories" for the renderer
        // Note: size calculation might be slightly off if maxScore isn't up to date, but that's acceptable
        renderStories(clippings, false);
    }
    ...
    function renderStories(stories, append = false) {
        ...
        stories.forEach((story, i) => {
            ...
            // Add Clipping Button
            const isClipped = appState.savedStories.has(story.id);
            const clipBtn = document.createElement('button');
            clipBtn.className = `clip-btn ${isClipped ? 'is-clipped' : ''}`;
            clipBtn.title = isClipped ? 'Unclip' : 'Clip to saved stories';
            clipBtn.setAttribute('aria-label', clipBtn.title);
            clipBtn.dataset.storyId = String(story.id);
            clipBtn.innerHTML = isClipped 
            ...
            // We use a closure here or delegation later. Delegation is cleaner but complex with Map.
            // Let's use simple event listener here for clarity
            clipBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                toggleSaveStory(story);
            });
            storyElement.appendChild(clipBtn);
            ...
            const commentsIcon = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path></svg>`;
            const commentsHTML = (commentsCount > 0)
                ? `<a class="comments-link" data-story-id="${story.id}">${commentsIcon}<span>${commentsCount}</span></a>`
                : `<span class="comments-link no-comments">${commentsIcon}<span>0</span></span>`;
            ...
            if (commentsLink && !commentsLink.classList.contains('no-comments')) {
        ...
        // DOM Cleanup: Remove old stories to keep performance high (only if not in clippings mode)
        if (appState.activeCategory !== 'clippings') {
    ...
    function updateSorterVisibility(category) {
        ...
        const isClippings = category === 'clippings';
        elements.sortButtons.forEach(button => {
            const sortType = button.dataset.sort;
            if (isClippings) {
                 button.hidden = true; // No sorting in clippings view yet
            } else {
                 button.hidden = isJobs && (sortType === 'score' || sortType === 'comments');
            }
        });
        if (isClippings) elements.subFilters.hidden = true;
    }
    ...
    function updateViewMode(category, isSearch = false) {
        ...
        elements.grid.classList.remove('serendipity-view');
        elements.subFilters.hidden = category === 'beststories' || category === 'clippings';
    ...
    function handleCategoryClick(e) {
        ...
        if (button.classList) button.classList.add('active'); // Might call with object if manual trigger
        
        elements.clippingsBtn.classList.remove('active'); // Deactivate clippings btn if category clicked
    ...
    function handleClippingsClick() {
        if (appState.activeCategory === 'clippings') return;
        
        appState.activeCategory = 'clippings';
        elements.categoryButtons.forEach(btn => btn.classList.remove('active'));
        elements.clippingsBtn.classList.add('active');
        elements.searchInput.value = '';
        
        updateViewMode('clippings');
        updateSorterVisibility('clippings');
        fetchStories(); // Will delegate to displayClippings
    }
    ...
    function handleSortClick(e) {
        ...
        if (appState.activeCategory && appState.activeCategory !== 'clippings') {
    ...
    function performSearch(query) {
        ...
        if (!appState.isSearchView) {
            elements.categoryButtons.forEach(btn => btn.classList.remove('active'));
            elements.clippingsBtn.classList.remove('active');
        }
    ...
    async function openCommentsOverlay(storyId) {
        ...
        try {
            ...
            if (story.url) {
                const domainName = new URL(story.url).hostname.replace('www.', '');
                metaParts.push(`<a href="${story.url}" target="_blank" rel="noopener noreferrer">${domainName}</a>`);
            }
            
            let storyTextHTML = '';
            if (story.text) {
                storyTextHTML = `<div class="overlay-story-text">${simpleSanitize(story.text)}</div>`;
            }

            elements.overlayStoryDetails.innerHTML = `
                <h2 class="story-title">${story.title}</h2>
                <div class="story-meta">
                    ${metaParts.join('<span class="separator">&bull;</span>')}
                </div>
                ${storyTextHTML}`;
    ...
    function setupInfiniteScroll() {
        const observer = new IntersectionObserver(([entry]) => {
            if (entry.isIntersecting && !appState.isFetching) {
                if (appState.activeCategory === 'clippings') return; // No infinite scroll for clippings
                if (appState.isSearchView) {
    ...
    function init() {
        ...
        elements.clippingsBtn.addEventListener('click', handleClippingsClick);
