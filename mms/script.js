document.addEventListener('DOMContentLoaded', () => {
    ...
    function applyRoleAccess(role) {
        ...
        const avatarContainer = document.querySelector('.nav-right');
        let roleSpan = document.getElementById('user-role-display');
        if (!roleSpan) {
            roleSpan = document.createElement('span');
            roleSpan.id = 'user-role-display';
            roleSpan.style.marginRight = '12px';
            roleSpan.style.fontWeight = '500';
            roleSpan.style.color = '#f8fafc';
            avatarContainer.insertBefore(roleSpan, avatarContainer.firstChild);
        }
        roleSpan.textContent = role;

        // Define access map
        const accessMap = {
            'Admin': ['nav-dashboard', 'nav-maintenance', 'nav-capabilities', 'nav-projects-with', 'nav-projects-without', 'nav-alerts', 'nav-sap', 'nav-reports'],
            'Packaging Development': ['nav-dashboard', 'nav-projects-with', 'nav-projects-without', 'nav-reports'],
            'Toolroom': ['nav-dashboard', 'nav-maintenance', 'nav-alerts'],
            'Procurement': ['nav-dashboard', 'nav-projects-with', 'nav-sap', 'nav-reports'],
            'QA': ['nav-dashboard', 'nav-projects-without', 'nav-reports'],
            'Plant/Operations': ['nav-dashboard', 'nav-maintenance', 'nav-alerts'],
            'Finance': ['nav-dashboard', 'nav-sap', 'nav-reports'],
            'Vendor': ['nav-dashboard', 'nav-projects-with'],
            'Auditor': ['nav-dashboard', 'nav-capabilities', 'nav-reports']
        };
        ...
        const navLinksMap = {
            ...
            'nav-maintenance': document.getElementById('nav-maintenance'),
            'nav-capabilities': document.getElementById('nav-capabilities'),
            'nav-projects-with': document.getElementById('nav-projects-with'),
            'nav-projects-without': document.getElementById('nav-projects-without'),
            'nav-alerts': document.getElementById('nav-alerts'),
            'nav-sap': document.getElementById('nav-sap'),
        ...
        allowed.forEach(key => {
            if (navLinksMap[key]) {
                navLinksMap[key].style.display = 'inline-block';
                if (!firstAllowedTarget && navLinksMap[key].getAttribute('data-target')) {
                    firstAllowedTarget = navLinksMap[key].getAttribute('data-target');
                }
            }
        });
        ...
        if (firstAllowedTarget) {
            ...
            document.querySelector(`[data-target="${firstAllowedTarget}"]`).classList.add('active');
            
            document.querySelectorAll('.view-container').forEach(view => {
                view.classList.add('hidden');
                view.classList.remove('active');
            });
            const targetView = document.getElementById(firstAllowedTarget);
            if (targetView) {
        ...
        if (role === 'Auditor') {
            // Read-only: disable buttons
            document.querySelectorAll('button, input, select').forEach(el => {
                if (el.id !== 'login-username' && el.id !== 'login-password' && el.id !== 'login-role' && el.type !== 'submit') {
                    el.disabled = true;
                    el.style.opacity = '0.6';
                    el.style.cursor = 'not-allowed';
                }
            });
            // Re-enable close modal button
            ...
            if(btnModalClose) {
                ...
                btnModalClose.style.opacity = '1';
                btnModalClose.style.cursor = 'pointer';
            }
        } else {
